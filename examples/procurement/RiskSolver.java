...
public class RiskSolver {
	...
	public static void main(String[] args) {
		...
		session = RiskSolver.factory.openSession();
		...
		try {
			...
			for (int k = 0; k < noTest; k++) {
				...
				RiskProblem riskProblem = new RiskProblem(1);

				NondominatedPopulation result = new Executor().withProblem(riskProblem).withAlgorithm(algo)
						.distributeOnAllCores().withMaxEvaluations(50000).run();
...
//					System.out.println();
	...
	// TEST
	public Long test(int project_id) {
		...
		try {
			...
			List result = cr.setProjection(Projections.countDistinct("id")).add(Restrictions.eq("projects.id", 1))
					.list();
